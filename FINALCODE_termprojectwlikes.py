...
def kMeansClusterAlg(app):
   ...
   if (len(app.buttonClickedLikedSongs) > 0):
       ...
       # Concatenate and drop duplicates to avoid redundancy
       liked_songs = pd.concat([liked_songs, new_liked_songs_df]).drop_duplicates()
 
   # print("liked song length new:")
   # print(len(liked_songs))
...
def redrawAll(app, canvas):
  ...
  if app.currentScreen == 'home':
      ...
      canvas.create_text(app.width//2, app.height//3 + 20, text="To upload data, make a spreadsheet with 3 columns", fill="black", font="Arial 16")
      #canvas.create_image(app.width//2, app.height//3 + 70, image=ImageTk.PhotoImage(app.sampleImage))
      canvas.create_text(app.width//2, app.height//3 + 40, text="(Song Number, Song Name, Song Artist). Then, download it as a .csv file!", fill="black", font="Arial 16")
      canvas.create_text(app.width//2, app.height//3 + 100, text="Once you press Upload Music Data, the model will begin training, and a", fill="black", font="Arial 16")
      canvas.create_text(app.width//2, app.height//3 + 120, text="K-Nearest Neighbors graph will pop up. The KNN graph uses your input", fill="black", font="Arial 16")
      canvas.create_text(app.width//2, app.height//3 + 140, text="song data to create a visual representation of the algorithm's", fill="black", font="Arial 16")
      canvas.create_text(app.width//2, app.height//3 + 160, text="clusters of the recommended, liked, and non-liked songs.", fill="black", font="Arial 16")
      canvas.create_text(app.width//2, app.height//3 + 220, text="Make sure you close the graph to continue.", fill="black", font="Arial 16")
